...
const Repos = ({ user, repos }) => {
  return (
    <div>
      <h2 className="font-bold text-2xl pt-4 pb-0">
      ...
      <div className="grid grid-cols-2 gap-1">
        {repos.map((repo) => {
          return (
            <div
              key={repo.id}
              className="rounded bg-white mx-8 p-4 hover:shadow-md hover:bg-blue-100 border-b-2"
            >
              ...
              <p>
                Language: {repo.language} / Starts: {repo.stargazer_count}{" "}
              </p>
            </div>
          );
        })}
